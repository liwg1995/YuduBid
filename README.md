...
围绕招标资料解析、投标机会扫描、目录生成、正文编排、已有方案扩写、可行性研究报告生成、知识库复用、标书查重和废标项检查形成完整辅助链路，让技术响应、项目论证、方案撰写、既有方案优化和交付检查更清晰。

招投标模块提供独立的 Word 模板管理能力。用户可以创建、预览、编辑、删除、导入和导出模板，并配置纸张与页边距、封面、页眉页脚、一级至九级标题、Word 自动多级编号或静态编号、正文段落、列表、表格与图片样式，以及图片和表格题注。技术方案、已有方案扩写和可行性研究报告导出时，可选择基础格式、`word-optimization` 优化排版或已保存的自定义模板；模板较多时支持按名称检索并快速选用。模板管理仅作用于招投标相关模块，不改变其他业务模块的 Word 导出配置。

可行性研究报告支持项目资料录入与导入、资料分析、报告目录及关键参数生成、正文编制与校审、AI/技术图谱/Mermaid 配图、整篇预览和 Word 导出；知识库资产可作为技术方案与可研报告流程中的素材沉淀与复用能力。
...
<p align="center">
  <img src="./docs/images/yudu-pm.png" alt="YuduBid 设置关于页中的禹都一只猫品牌标识" width="220" />
</p>
