...
const bridge = {
  ...
  agent: {
    ...
    runDry: (input) => invoke('agent:run-dry', input),
    runShadow: (input) => invoke('agent:run-shadow', input),
    getShadowReport: (options) => invoke('agent:get-shadow-report', options),
    previewOutlineGeneration: (input) => invoke('agent:preview-outline-generation', input),
    createOutlineApprovalDraft: (input) => invoke('agent:create-approval-draft', input),
    listApprovalDrafts: (options) => invoke('agent:list-approval-drafts', options),
    revokeApprovalDraft: (draftId) => invoke('agent:revoke-approval-draft', draftId),
    getApprovalDraftIntegrity: (options) => invoke('agent:get-approval-draft-integrity', options),
    getApprovalDraftAuditLog: (options) => invoke('agent:get-approval-draft-audit-log', options),
    previewApprovalPackage: (draftId) => invoke('agent:preview-approval-package', draftId),
    validateApprovalPackage: (approvalPackage) => invoke('agent:validate-approval-package', approvalPackage),
    getContinuousRun: (input) => invoke('agent:get-continuous-run', input),
    saveContinuousRun: (input) => invoke('agent:save-continuous-run', input),
  },
  ai: {
