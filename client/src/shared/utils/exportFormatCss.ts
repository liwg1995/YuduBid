...
import type { ExportFormatConfig, HeadingStyleConfig, LineSpacingMode, LineSpacingUnit, ListStyle, OrderedListStyle, PaperSize, ParagraphSpacingUnit } from '../types/exportFormat';
...
function paragraphSpacingToCss(value: number, unit: ParagraphSpacingUnit): string {
  if (unit === 'auto') return '0';
  if (unit === 'line') return `${value}em`;
  return `${value}${unit}`;
}

function measurementToPt(value: number, unit: LineSpacingUnit): number {
...
function lineSpacingToCss(value: number, mode: LineSpacingMode, unit: LineSpacingUnit): string {
  if (mode === 'single') return '1';
  if (mode === 'one-and-half') return '1.5';
  if (mode === 'double') return '2';
  if (mode === 'multiple') return String(value);
  return `${measurementToPt(value, unit)}pt`;
}
...
function buildHeadingVars(level: number, config: HeadingStyleConfig): Record<string, string> {
  ...
  return {
    ...
    [`--ef-h${n}-spacing-before`]: paragraphSpacingToCss(config.spacing_before_pt, config.spacing_before_unit),
    [`--ef-h${n}-spacing-after`]: paragraphSpacingToCss(config.spacing_after_pt, config.spacing_after_unit),
    [`--ef-h${n}-indent`]: '0',
    [`--ef-h${n}-line-height`]: lineSpacingToCss(config.line_spacing, config.line_spacing_mode, config.line_spacing_unit),
  };
}
...
export function buildExportFormatCssVars(config: ExportFormatConfig): Record<string, string> {
  ...
  vars['--ef-body-spacing-before'] = paragraphSpacingToCss(config.body_text.spacing_before_pt, config.body_text.spacing_before_unit);
  vars['--ef-body-spacing-after'] = paragraphSpacingToCss(config.body_text.spacing_after_pt, config.body_text.spacing_after_unit);
  vars['--ef-body-indent'] = config.body_text.first_line_indent_chars > 0
    ? `${config.body_text.first_line_indent_chars}em`
    : '0';
  vars['--ef-body-line-height'] = lineSpacingToCss(config.body_text.line_spacing_multiple, config.body_text.line_spacing_mode, config.body_text.line_spacing_unit);
  ...
  if (table) {
    ...
    vars['--ef-table-caption-size'] = `${chineseSizeToPt(table.caption_size || '小五')}pt`;
