...
const thesisTutorUsageSteps = [
  {
    title: '先补论文档案',
    description: '学位、类型、专业、方向、阶段、引用格式和题目会作为全流程上下文。后续每个模块生成时都会带入这些信息，用来控制难度、口吻、学科知识和输出结构。',
  },
  {
    title: '从启动诊断开始',
    description: '第一次建议先做启动诊断，确认阶段、卡点和路线。诊断后如果题目、方向、阶段基本确定，可以锁定档案；后续模块会按这份档案持续推进。',
  },
  {
    title: '按模块继续推进',
    description: '推荐顺序是：启动诊断 → 选题与开题 → 文献综述 → 研究设计 → 逐章写作 → 评审与答辩 → 格式与查重。也可以根据当前卡点直接进入对应模块。',
  },
  {
    title: '需求写目标，材料放依据',
    description: '“本次需求”写清你希望导师做什么；“材料区”放导师意见、文献摘要、政策案例、数据说明、访谈记录或正文草稿。材料越真实，结果越能落到论文上。',
  },
  {
    title: '生成后要编辑和沉淀',
    description: '结果区可以继续手动修改、保存、复制或导出 Word。阶段成果可以带入后续模块，也可以沉淀到论文档案、证据链、章节、反馈或检查清单。',
  },
  {
    title: '它是辅导，不是代写',
    description: '论文导师帮助你规划路径、整理材料、设计结构、修改表达和检查风险；最终论文仍要基于真实研究、导师意见和学校规范完成。',
  },
];
...
const thesisTutorNoticeItems = [
  '论文导师不是“全盘代写论文”的工具，它更适合帮你拆解问题、规划路径、组织材料、检查逻辑和改写表达。',
...
const referenceEnabledPanels = new Set<ThesisTutorPanel>(['literature', 'writing', 'review', 'format']);
const feedbackEnabledPanels = new Set<ThesisTutorPanel>(['writing', 'review', 'format']);
...
function ThesisTutorPage({ initialPanel = 'diagnosis', onNavigate }: ThesisTutorPageProps) {
  ...
  const chapterSummary = activePanel === 'writing'
    ...
    <section className="thesis-tutor-panel thesis-tutor-profile-panel">
      <div className="thesis-tutor-panel-head thesis-tutor-profile-head">
        <div>
          ...
          <span>建议先在启动诊断阶段确认一次；后续选题、综述、研究设计、写作和答辩都会沿用这份档案。</span>
        </div>
  ...
  function selectChapter(chapterId: string) {
    ...
    if (activePanel === 'writing' && nextChapter?.draft) {
  ...
  function createDefaultCheckItems() {
    if (checkItems.length) {
      showToast('已有检查项，可继续新增单项检查', 'info');
      return;
    }
    ...
    const nextChecks = defaults.map((item) => ({
      ...createLocalCheckItem(item.title),
      ...item,
      status: 'unchecked' as ThesisTutorCheckStatus,
    }));
    setCheckItems(nextChecks);
    setActiveCheckId(nextChecks[0]?.id || '');
  }
  ...
  function getMaterialExtractLabel() {
    ...
    if (activePanel === 'writing') return '放入章节材料';
  ...
  function switchPanel(nextPanel: ThesisTutorPanel) {
    ...
    } else if (nextPanel === 'writing' && activeChapter?.draft) {
  ...
  async function importWorkspace() {
    ...
    const requestId = startOperationProgress('正在导入论文导师备份');
    try {
      setSaving(true);
      const payload = await window.yibiao.thesisTutor.importWorkspace();
      if (payload.success) {
        applyWorkspaceState(payload.state);
        finishOperationProgress(requestId, payload.message || '论文导师工作区备份已导入');
        showToast(payload.message || '论文导师工作区备份已导入', 'success');
      } else if (!payload.canceled) {
    ...
    <div className="thesis-tutor-page" ref={mainScrollRef} onWheelCapture={handleMainWheelCapture}>
      <header className="thesis-tutor-header">
        ...
        <div className="thesis-tutor-actions">
          ...
          <button type="button" className="secondary-action" onClick={importWorkspace} disabled={saving || isRunning}>导入备份</button>
          <Dialog.Root>
            <Dialog.Trigger asChild>
      ...
      <section className="thesis-tutor-help-strip">
        <div>
          ...
          <span>按“论文档案 → 选择模块 → 填需求和材料 → 生成并编辑”的顺序使用。</span>
        </div>
        <Dialog.Root>
          <Dialog.Trigger asChild>
            ...
            <Dialog.Content className="thesis-tutor-help-card">
              <div className="thesis-tutor-help-head">
                <div>
                  ...
                  <Dialog.Description>
                    论文导师是一个论文全过程工作台，适合从定位阶段、选题开题、文献综述、研究设计到写作修改和答辩检查的连续辅导。
                  </Dialog.Description>
                </div>
              ...
              <div className="thesis-tutor-help-lock">
                ...
                <p>论文档案不是独立表单，而是后续生成的上下文。启动诊断后，如果学位、专业、方向、题目和阶段基本确定，可以点击“锁定档案”，避免后续误改；如果题目或方向发生变化，再解锁调整。</p>
              </div>
              <div className="thesis-tutor-help-tip">
                小建议：如果结果太泛，通常不是模块选错，而是“本次需求”和“材料区”太少。把导师要求、文献摘要、数据说明或真实草稿贴进去，回复会明显更贴近论文；材料不足时，它会优先给结构、路径和待补充清单。
              </div>
              <div className="thesis-tutor-help-actions">
                <Dialog.Close className="primary-action" type="button">知道了</Dialog.Close>
              </div>
            ...
            <div className="thesis-tutor-panel thesis-tutor-check-panel">
              <div className="thesis-tutor-panel-head">
                ...
                <div className="thesis-tutor-chapter-actions">
                  <button type="button" className="secondary-action" onClick={createDefaultCheckItems} disabled={saving || isRunning}>生成常见检查项</button>
              ...
              {checkItems.length ? (
                ...
                <div className="thesis-tutor-chapter-empty">
                  <p>还没有检查项。可以先生成常见检查项，也可以根据学校模板、查重报告或导师意见新增单项检查。</p>
                  <div className="thesis-tutor-empty-actions">
                    <button type="button" className="secondary-action" onClick={createDefaultCheckItems} disabled={saving || isRunning}>生成常见检查项</button>
          ...
          {activePanel === 'writing' && (
            <div className="thesis-tutor-panel thesis-tutor-chapter-panel">
              <div className="thesis-tutor-panel-head">
                <div>
                  <strong>章节工作区</strong>
                  <span>逐章写作会优先参考当前章节的目标、材料、导师反馈和已保存草稿。</span>
                </div>
                <div className="thesis-tutor-chapter-actions">
